...
# -*- coding: utf-8 -*-
"""
Animedia FUN DUB Bot
Muallif: @yolgizyigit7
"""
...
from aiogram.types import (
    ...
    InlineKeyboardButton, CallbackQuery, FSInputFile, BufferedInputFile
)
...
# ================================================================
# KONFIGURATSIYA (.env dan olinadi)
# ================================================================
BOT_TOKEN = os.getenv("BOT_TOKEN")
OWNERS = [int(x.strip()) for x in os.getenv("OWNERS", "").split(",") if x.strip()]
ADMINS = OWNERS.copy()  # Adminlar = Ownerlar
MAIN_CHANNEL = os.getenv("MAIN_CHANNEL", "@animedia_fandub")
AUTHOR_USERNAME = os.getenv("AUTHOR_USERNAME", "@yolgizyigit7")
AUTHOR_LINK = os.getenv("AUTHOR_LINK", "https://t.me/yolgizyigit7")
SUPPORT_USERNAME = os.getenv("SUPPORT_USERNAME", "@yolgizyigit7")
SUPPORT_LINK = os.getenv("SUPPORT_LINK", "https://t.me/yolgizyigit7")
BASE_CHANNEL_ID = int(os.getenv("BASE_CHANNEL_ID", "-1003988599093"))
DB_NAME = os.getenv("DATABASE_NAME", "anime.db")
...
# Majburiy kanallar jadvali (endi Instagram va YouTube ham qo'shiladi)
cursor.execute('''
...
# Ownerlarni adminlarga qo'shish
...
def start_menu():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="⭐ Reyting berish", callback_data="rate_anime")],
...
def forced_channel_buttons():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="➕ Telegram kanal", callback_data="forced_add_telegram")],
        [InlineKeyboardButton(text="📸 Instagram", callback_data="forced_add_instagram")],
        [InlineKeyboardButton(text="▶️ YouTube", callback_data="forced_add_youtube")],
        [InlineKeyboardButton(text="❌ Kanal chiqarish", callback_data="forced_remove")],
        [InlineKeyboardButton(text="🔙 Orqaga", callback_data="back_to_admin_reply")]
    ])

def forced_channel_list_keyboard(page=0):
    builder = InlineKeyboardBuilder()
    cursor.execute("SELECT id, channel_username, channel_type, is_active FROM forced_channels ORDER BY channel_type, channel_username")
    channels = cursor.fetchall()
    per_page = 10
    start = page * per_page
    for ch_id, channel, ch_type, active in channels[start:start+per_page]:
        status = "✅" if active else "❌"
        type_emoji = "📢" if ch_type == "telegram" else "📸" if ch_type == "instagram" else "▶️"
        builder.button(text=f"{status} {type_emoji} {channel[:20]}", callback_data=f"forced_remove_{ch_id}")
    builder.adjust(1)
    if page > 0:
        builder.row(InlineKeyboardButton(text="⬅️", callback_data=f"forced_page_{page-1}"))
    if start+per_page < len(channels):
        builder.row(InlineKeyboardButton(text="➡️", callback_data=f"forced_page_{page+1}"))
    builder.row(InlineKeyboardButton(text="🔙 Orqaga", callback_data="back_to_forced_menu"))
...
def watch_parts_keyboard(media_id, page=0):
    ...
    if start+per_page < len(parts):
        builder.row(InlineKeyboardButton(text="➡️", callback_data=f"watch_parts_page_{media_id}_{page+1}"))
    builder.row(InlineKeyboardButton(text="⭐ Reyting", callback_data=f"rate_media_{media_id}"))
...
def rating_keyboard(media_id):
    builder = InlineKeyboardBuilder()
    for i in range(1, 11):
        builder.button(text=f"{i}⭐", callback_data=f"rating_{media_id}_{i}")
    builder.adjust(5)
    builder.row(InlineKeyboardButton(text="🔙 Orqaga", callback_data=f"watch_parts_{media_id}"))
    return builder.as_markup()
...
def get_media_rating(media_id):
    cursor.execute("SELECT AVG(rating), COUNT(*) FROM ratings WHERE media_id = ?", (media_id,))
    avg, count = cursor.fetchone()
    if avg is None:
        return 0.0, 0
    return round(avg, 1), count

def format_media_post(media):
    """Media ma'lumotlarini chiroyli formatda chiqarish"""
    name, media_type, genre, status, season, total_parts, views, code, voice, sponsor, quality, rating, rating_count = media
    ...
    sponsor_text = sponsor if sponsor else "Animedia FUN DUB"
    
    # Janrlarni # bilan formatlash
    genre_list = [f"#{g.strip().capitalize()}" for g in genre.split(",")] if genre else []
    genre_text = " ".join(genre_list) if genre_list else "Noma'lum"
...
📊 Holati : {status_text}
. . . ── . . . . . . . . . . . . . . . . . . . .
🎙 Ovoz berdi : {voice_text}
. . . ── . . . . . . . . . . . . . . . . . . . .
💿 Sifat : {quality}
. . . ── . . . . . . . . . . . . . . . . . . . .
💭 Tili : O'zbek tilida
. . . ── . . . . . . . . . . . . . . . . . . . .
⭐️ Reyting : {rating}/10 ({rating_count} ta baho)
✽───〔•°⛩°•〕───✽
...
def format_part_post(media_name, part_num, code):
    """Qism postini chiroyli formatda chiqarish"""
    return f"""✽───〔•°⛩°•〕───✽
🏷 Anime nomi : {media_name}
└─🎬 {part_num}-qism
✽───〔•°⛩°•〕───✽
...
async def check_subscription(user_id):
    """Foydalanuvchi majburiy kanallarga a'zoligini tekshirish"""
    cursor.execute("SELECT channel_username, channel_type FROM forced_channels WHERE is_active = 1")
    channels = cursor.fetchall()
    if not channels:
        return True, []
        ...
        if channel_type == "telegram":
            ...
            try:
                ...
                if member.status in ['left', 'kicked']:
                    not_subscribed.append({"type": channel_type, "username": channel_username})
            except Exception as e:
                print(f"⚠️ Kanal tekshirish xatosi {channel_username}: {e}")
        else:
            # Instagram va YouTube uchun faqat link ko'rsatiladi, tekshirilmaydi
            # Foydalanuvchi o'zi a'zo bo'lishi kerak
            not_subscribed.append({"type": channel_type, "username": channel_username})
    
    return len(not_subscribed) == 0, not_subscribed

async def get_subscription_keyboard():
    """Majburiy kanallar uchun tugmalar yaratish (username KO'RSATILMAYDI)"""
    cursor.execute("SELECT channel_username, channel_type FROM forced_channels WHERE is_active = 1")
    channels = cursor.fetchall()
        ...
        if channel_type == "telegram":
            ...
            if clean_channel:
                keyboard.inline_keyboard.append([
                    InlineKeyboardButton(text="📢 Kanalga a'zo bo'ling", url=f"https://t.me/{clean_channel}")
                ])
        elif channel_type == "instagram":
            clean_channel = channel_username.replace('@', '').replace('instagram.com/', '').strip()
            keyboard.inline_keyboard.append([
                InlineKeyboardButton(text="📸 Instagram da kuzatib boring", url=f"https://instagram.com/{clean_channel}")
            ])
        elif channel_type == "youtube":
            keyboard.inline_keyboard.append([
                InlineKeyboardButton(text="▶️ YouTube kanaliga obuna bo'ling", url=channel_username)
            ])
...
async def safe_send_video(chat_id, video, caption=None, **kwargs):
    try:
        return await bot.send_video(chat_id, video, caption=caption, **kwargs)
    except:
        return None

async def send_text_file(chat_id, text, filename):
    try:
...
# ========== MEDIA KO'RISH ==========
async def view_media_by_id(message, media_id):
    try:
        cursor.execute("SELECT name, type, description, image_url, genre, status, season, total_parts, views, code, voice, sponsor, quality, rating, rating_count FROM media WHERE id = ?", (media_id,))
        media = cursor.fetchone()
        if not media:
            await safe_send_message(message.chat.id, "❌ Media topilmadi!")
            return
        name, media_type, desc, image, genre, status, season, total_parts, views, code, voice, sponsor, quality, rating, rating_count = media
        cursor.execute("UPDATE media SET views = views + 1 WHERE id = ?", (media_id,))
        conn.commit()
        
        post = format_media_post((name, media_type, genre, status, season, total_parts, views+1, code, voice, sponsor, quality, rating, rating_count))
        
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="📺 Tomosha qilish", callback_data=f"watch_parts_{media_id}")],
            [InlineKeyboardButton(text="⭐ Reyting", callback_data=f"rate_media_{media_id}")],
            [InlineKeyboardButton(text="🔙 Ortga", callback_data="back_to_start")]
        ])
        
        if image and (image.startswith("http") or image.startswith("AgA")):
            await safe_send_photo(message.chat.id, photo=image, caption=post, reply_markup=keyboard, parse_mode="HTML")
        else:
            await safe_send_message(message.chat.id, post, reply_markup=keyboard, parse_mode="HTML")
    except Exception as e:
        print(f"view_media_by_id xatosi: {e}")
...
async def start(message: types.Message):
    ...
    if not is_subscribed and not_subscribed:
        text = "❌ Botdan foydalanish uchun quyidagi manbalarga a'zo bo'ling:\n\n"
        for ch in not_subscribed:
            if ch["type"] == "telegram":
                text += f"📢 Telegram kanal\n"
            elif ch["type"] == "instagram":
                text += f"📸 Instagram\n"
            elif ch["type"] == "youtube":
                text += f"▶️ YouTube\n"
            ...
            try:
                ...
                if media:
                    ...
                    if part:
                        ...
                        full_caption = format_part_post(media_name, part_num_int, code_int)
    ...
    welcome_text = (
        "🎬 <b>Animedia FUN DUB Rasmiy Bot</b> 🎬\n\n"
...
async def check_subscription_callback(callback: CallbackQuery):
    ...
    if is_subscribed:
        ...
        if not_subscribed:
            text = "❌ Hali ham quyidagi manbalarga a'zo emassiz:\n\n"
            for ch in not_subscribed:
                if ch["type"] == "telegram":
                    text += f"📢 Telegram kanal\n"
                elif ch["type"] == "instagram":
...
async def guide_start(callback: CallbackQuery):
    ...
    text = (
        ...
        "⭐ Reyting berish - Animelarga 1-10 gacha baho berish\n"
...
@dp.callback_query(F.data == "rate_anime")
async def rate_anime_list(callback: CallbackQuery, state: FSMContext):
    await update_user_activity(callback.from_user.id)
    await state.set_state(RatingState.waiting_for_rating)
    
    # Barcha animelarni ko'rsatish
    builder = InlineKeyboardBuilder()
    cursor.execute("SELECT id, name, code FROM media WHERE type = 'anime' ORDER BY name")
    anime_list = cursor.fetchall()
    
    if not anime_list:
        await callback.answer("Hozircha anime mavjud emas!", show_alert=True)
        return
    
    for media_id, name, code in anime_list[:30]:
        builder.button(text=f"{name} [{code}]", callback_data=f"rate_select_{media_id}")
    builder.adjust(1)
    builder.row(InlineKeyboardButton(text="🔙 Orqaga", callback_data="back_to_start"))
    
    await callback.message.edit_text("⭐ Reyting bermoqchi bo'lgan animeni tanlang:", reply_markup=builder.as_markup())
    await callback.answer()

@dp.callback_query(F.data.startswith("rate_select_"))
async def rate_select_media(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        f"⭐ <b>{media_name}</b> ga qancha baho berasiz?\n\n1⭐ - 10⭐ gacha tanlang:",
...
async def set_rating(callback: CallbackQuery, state: FSMContext):
    ...
    cursor.execute("SELECT id FROM ratings WHERE user_id = ? AND media_id = ?", (user_id, media_id))
    existing = cursor.fetchone()
    
    if existing:
        cursor.execute("UPDATE ratings SET rating = ?, created_at = ? WHERE user_id = ? AND media_id = ?",
                       (rating, datetime.now().isoformat(), user_id, media_id))
    else:
        cursor.execute("INSERT INTO ratings (user_id, media_id, rating, created_at) VALUES (?, ?, ?, ?)",
                       (user_id, media_id, rating, datetime.now().isoformat()))
    
    # Media jadvalidagi o'rtacha reytingni yangilash
    avg_rating, count = get_media_rating(media_id)
    cursor.execute("UPDATE media SET rating = ?, rating_count = ? WHERE id = ?", (avg_rating, count, media_id))
    conn.commit()
    
    cursor.execute("SELECT name FROM media WHERE id = ?", (media_id,))
    media_name = cursor.fetchone()[0]
    
    await callback.message.edit_text(f"✅ <b>{media_name}</b> ga {rating}⭐ baho berdingiz!\n\nReyting: {avg_rating}/10 ({count} ta baho)", parse_mode="HTML")
    await state.clear()
    await callback.answer()
...
async def search_media_query(message: types.Message, state: FSMContext):
...
async def view_media(callback: CallbackQuery):
    try:
        ...
        if image and (image.startswith("http") or image.startswith("AgA")):
            await safe_send_photo(callback.from_user.id, photo=image, caption=post, reply_markup=keyboard, parse_mode="HTML")
        else:
            await safe_send_message(callback.from_user.id, post, reply_markup=keyboard, parse_mode="HTML")
        try:
            await callback.message.delete()
        except:
            pass
        ...
        print(f"view_media xatosi: {e}")
        await callback.answer("Xatolik yuz berdi!")

@dp.callback_query(F.data.startswith("rate_media_"))
async def rate_media(callback: CallbackQuery):
    media_id = int(callback.data.split("_")[2])
    cursor.execute("SELECT name FROM media WHERE id = ?", (media_id,))
    media_name = cursor.fetchone()[0]
    await callback.message.edit_text(
...
async def watch_part(callback: CallbackQuery):
    try:
        ...
        cursor.execute("SELECT name, code FROM media WHERE id = ?", (media_id,))
        media_name, code = cursor.fetchone()
        full_caption = format_part_post(media_name, part_num, code)
        await safe_send_video(callback.from_user.id, video=file_id, caption=full_caption, parse_mode="HTML")
        await callback.answer()
    except Exception as e:
        print(f"watch_part xatosi: {e}")
        await callback.answer("Xatolik yuz berdi!")
...
async def watch_parts_page(callback: CallbackQuery):
    try:
        ...
        print(f"watch_parts_page xatosi: {e}")
        await callback.answer("Xatolik yuz berdi!")
...
async def back_to_main_reply(message: types.Message):
    start_image = get_start_image()
    welcome_text = (
        "🎬 <b>Animedia FUN DUB Rasmiy Bot</b> 🎬\n\n"
        "✨ <b>Botimizga xush kelibsiz!</b> ✨\n\n"
...
async def back_to_admin_reply(callback: CallbackQuery):
    ...
    try:
        ...
        pass
    await safe_send_message(callback.from_user.id, "🔐 Admin Panel", reply_markup=admin_menu(), parse_mode="HTML")
    await callback.answer()
...
async def add_media_code(message: types.Message, state: FSMContext):
    try:
...
async def add_media_description(message: types.Message, state: FSMContext):
...
async def add_media_image_photo(message: types.Message, state: FSMContext):
    await state.update_data(image=message.photo[-1].file_id)
    await message.answer("Janrlarini kiriting (vergul bilan, masalan: Romance, Drama, Action):")
    await state.set_state(AddMediaState.genre)

@dp.message(AddMediaState.image, F.text)
async def add_media_image_url(message: types.Message, state: FSMContext):
    await state.update_data(image=message.text)
    await message.answer("Janrlarini kiriting (vergul bilan, masalan: Romance, Drama, Action):")
    await state.set_state(AddMediaState.genre)

@dp.message(AddMediaState.genre)
async def add_media_genre(message: types.Message, state: FSMContext):
...
async def add_media_season(message: types.Message, state: FSMContext):
    try:
        season = int(message.text.strip())
        await state.update_data(season=season)
        await message.answer("Ovoz beruvchi(lar)ni kiriting (masalan: Animedia FUN DUB):")
        await state.set_state(AddMediaState.voice)
    except ValueError:
        await message.answer("❌ Faqat raqam kiriting!")

@dp.message(AddMediaState.voice)
async def add_media_voice(message: types.Message, state: FSMContext):
    await state.update_data(voice=message.text)
    await message.answer("Himoy (homiy) ni kiriting (masalan: Animedia):")
    await state.set_state(AddMediaState.sponsor)

@dp.message(AddMediaState.sponsor)
async def add_media_sponsor(message: types.Message, state: FSMContext):
    await state.update_data(sponsor=message.text)
    await message.answer("Sifatni kiriting (masalan: 720p, 1080p):")
    await state.set_state(AddMediaState.quality)

@dp.message(AddMediaState.quality)
async def add_media_quality(message: types.Message, state: FSMContext):
    await state.update_data(quality=message.text)
    data = await state.get_data()
    try:
        cursor.execute("INSERT INTO media (code, type, name, description, image_url, genre, status, season, voice, sponsor, quality, created_at, rating, rating_count) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                       (data['code'], data['type'], data['name'], data['description'], data['image'], data['genre'], data['status'], data['season'], data['voice'], data['sponsor'], data['quality'], datetime.now().isoformat(), 0.0, 0))
        conn.commit()
...
async def add_part_select_media(message: types.Message, state: FSMContext):
    query = message.text.strip()
    try:
        code = int(query)
...
async def add_part_number(message: types.Message, state: FSMContext):
    try:
...
async def add_part_caption(message: types.Message, state: FSMContext):
...
async def add_multiple_parts_start(message: types.Message, state: FSMContext):
    if not is_admin(message.from_user.id): return
    await message.answer("Qaysi animega qism qo'shmoqchisiz?\nAnime nomi yoki kodini kiriting:", parse_mode="HTML")
    await state.set_state(AddMultiplePartsState.select_media)

@dp.message(AddMultiplePartsState.select_media)
async def add_multiple_parts_select_media(message: types.Message, state: FSMContext):
    query = message.text.strip()
    try:
        code = int(query)
        cursor.execute("SELECT id, name FROM media WHERE code = ?", (code,))
...
async def add_multiple_parts_video(message: types.Message, state: FSMContext):
    data = await state.get_data()
...
async def add_multiple_parts_done(message: types.Message, state: FSMContext):
    data = await state.get_data()
    ...
    for video in videos:
        ...
        if not cursor.fetchone():
            ...
            try:
                await bot.send_video(BASE_CHANNEL_ID, video=video['file_id'], caption=f"{video['part_number']}-qism")
            except:
                pass
            saved += 1
    if saved > 0:
        ...
        conn.commit()
    await message.answer(f"✅ {saved} ta qism qo'shildi va bazaga yuborildi!", parse_mode="HTML")
    await state.clear()

# ========== MEDIA TAHRIRLASH ==========
@dp.message(F.text == "✏️ Media Tahrirlash")
async def edit_media_start(message: types.Message, state: FSMContext):
    if not is_admin(message.from_user.id): return
...
async def edit_media_select(message: types.Message, state: FSMContext):
    query = message.text.strip()
    try:
        code = int(query)
        cursor.execute("SELECT id, name FROM media WHERE code = ?", (code,))
...
async def edit_media_value(message: types.Message, state: FSMContext):
    data = await state.get_data()
    media_id = data['media_id']
    field = data['field']
    value = message.text
...
async def edit_media_image_photo(message: types.Message, state: FSMContext):
    data = await state.get_data()
...
async def edit_part_start(message: types.Message, state: FSMContext):
    if not is_admin(message.from_user.id): return
...
async def edit_part_select_media(message: types.Message, state: FSMContext):
    query = message.text.strip()
    try:
        code = int(query)
        cursor.execute("SELECT id, name FROM media WHERE code = ?", (code,))
...
async def edit_part_video(message: types.Message, state: FSMContext):
    data = await state.get_data()
...
async def edit_part_value(message: types.Message, state: FSMContext):
    data = await state.get_data()
...
async def show_stats(message: types.Message):
    ...
    ratings = cursor.execute("SELECT COUNT(*) FROM ratings").fetchone()[0]
...
# ========== MAJBURIY A'ZO (ADMIN PANEL) ==========
@dp.message(F.text == "🔗 Majburiy A'zo")
async def forced_subscribe_menu(message: types.Message, state: FSMContext):
    if not is_admin(message.from_user.id): return
    await message.answer("🔗 Majburiy a'zolik boshqaruvi:", reply_markup=forced_channel_buttons(), parse_mode="HTML")
    await state.set_state(ForcedChannelState.action)

@dp.callback_query(ForcedChannelState.action, F.data == "forced_add_telegram")
async def forced_add_telegram(callback: CallbackQuery, state: FSMContext):
    await state.update_data(action="add", channel_type="telegram")
    try:
        await callback.message.edit_text("➕ Telegram kanal username yoki linkini yuboring:\nMasalan: @kanal yoki https://t.me/kanal")
    except:
        await safe_send_message(callback.from_user.id, "➕ Telegram kanal username yoki linkini yuboring:\nMasalan: @kanal yoki https://t.me/kanal")
    await callback.answer()

@dp.callback_query(ForcedChannelState.action, F.data == "forced_add_instagram")
async def forced_add_instagram(callback: CallbackQuery, state: FSMContext):
    await state.update_data(action="add", channel_type="instagram")
    try:
        await callback.message.edit_text("📸 Instagram username yoki linkini yuboring:\nMasalan: @username yoki https://instagram.com/username")
    except:
        await safe_send_message(callback.from_user.id, "📸 Instagram username yoki linkini yuboring:\nMasalan: @username yoki https://instagram.com/username")
    await callback.answer()

@dp.callback_query(ForcedChannelState.action, F.data == "forced_add_youtube")
async def forced_add_youtube(callback: CallbackQuery, state: FSMContext):
    await state.update_data(action="add", channel_type="youtube")
    try:
        await callback.message.edit_text("▶️ YouTube kanal linkini yuboring:\nMasalan: https://youtube.com/c/kanal")
    except:
        await safe_send_message(callback.from_user.id, "▶️ YouTube kanal linkini yuboring:\nMasalan: https://youtube.com/c/kanal")
    await callback.answer()

@dp.callback_query(ForcedChannelState.action, F.data == "forced_remove")
async def forced_remove_start(callback: CallbackQuery, state: FSMContext):
...
@dp.callback_query(F.data.startswith("forced_remove_"))
async def forced_remove_channel(callback: CallbackQuery, state: FSMContext):
    channel_id = int(callback.data.split("_")[2])
    cursor.execute("DELETE FROM forced_channels WHERE id = ?", (channel_id,))
    conn.commit()
    try:
        await callback.message.edit_text("✅ Kanal o'chirildi!", reply_markup=forced_channel_list_keyboard())
    except:
        await safe_send_message(callback.from_user.id, "✅ Kanal o'chirildi!", reply_markup=forced_channel_list_keyboard())
    await callback.answer()

@dp.callback_query(F.data.startswith("forced_page_"))
async def forced_page_callback(callback: CallbackQuery, state: FSMContext):
...
async def back_to_forced_menu(callback: CallbackQuery, state: FSMContext):
    try:
...
@dp.message(ForcedChannelState.action, F.text)
async def forced_add_channel(message: types.Message, state: FSMContext):
    data = await state.get_data()
    ...
    if channel_type == "telegram":
        ...
        if channel_input.startswith("https://instagram.com/"):
            channel = "@" + channel_input.split("/")[-1]
        elif channel_input.startswith("https://www.instagram.com/"):
            channel = "@" + channel_input.split("/")[-1]
        elif channel_input.startswith("@"):
            channel = channel_input
        else:
    ...
    try:
        cursor.execute("INSERT INTO forced_channels (channel_username, channel_type, is_active) VALUES (?, ?, ?)",
                       (channel, channel_type, 1))
        conn.commit()
        type_text = "Telegram" if channel_type == "telegram" else "Instagram" if channel_type == "instagram" else "YouTube"
        await message.answer(f"✅ {type_text} kanali majburiy a'zolik ro'yxatiga qo'shildi!")
    except sqlite3.IntegrityError:
        await message.answer(f"⚠️ Bu kanal allaqachon ro'yxatda!")
    
    await state.clear()

# ========== ADMIN QO'SHISH/CHIQARISH ==========
...
async def admin_remove_request(callback: CallbackQuery, state: FSMContext):
    if not is_owner(callback.from_user.id):
        ...
        return
    await state.update_data(action="remove")
    admins = cursor.execute("SELECT user_id FROM admins WHERE user_id NOT IN ({})".format(','.join('?' for _ in OWNERS)), OWNERS).fetchall()
    if admins:
...
async def admin_manage_user_id(message: types.Message, state: FSMContext):
    try:
...
async def post_media_id(message: types.Message, state: FSMContext):
    query = message.text.strip()
    try:
        code = int(query)
        cursor.execute("SELECT id, name, code, description, total_parts, status, season, genre, voice, sponsor, quality, image_url, rating, rating_count FROM media WHERE code = ?", (code,))
    except ValueError:
        cursor.execute("SELECT id, name, code, description, total_parts, status, season, genre, voice, sponsor, quality, image_url, rating, rating_count FROM media WHERE name LIKE ?", (f"%{query}%",))
    media = cursor.fetchone()
    if not media:
        await message.answer(f"❌ '{query}' bo'yicha media topilmadi!")
        return
    media_id, name, code, desc, total_parts, status, season, genre, voice, sponsor, quality, image, rating, rating_count = media
    await state.update_data(media_id=media_id, media_name=name, media_code=code, media_image=image)
    
    post = format_media_post((name, "", genre, status, season, total_parts, 0, code, voice, sponsor, quality, rating, rating_count))
    
    await message.answer(
        f"📨 <b>Quyidagi post yuboriladi:</b>\n\n{post}\n\n"
...
async def post_channel(message: types.Message, state: FSMContext):
    ...
    cursor.execute("SELECT total_parts, status, season, genre, voice, sponsor, quality, rating, rating_count FROM media WHERE id = ?", (media_id,))
    total_parts, status, season, genre, voice, sponsor, quality, rating, rating_count = cursor.fetchone()
...
async def post_confirm(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        if image and (image.startswith("http") or image.startswith("AgA")):
            await bot.send_photo(chat_id=channel, photo=image, caption=post, reply_markup=keyboard, parse_mode="HTML")
        else:
            await bot.send_message(chat_id=channel, text=post, reply_markup=keyboard, parse_mode="HTML")
        try:
...
# ========== QISMNI POST QILISH (YANGI FORMAT) ==========
@dp.message(F.text == "🎬 Qismni Post Qilish")
async def part_post_start(message: types.Message, state: FSMContext):
    if not is_admin(message.from_user.id): return
    await message.answer("🎬 Qismni post qilish\n\nPost qilmoqchi bo'lgan media nomi yoki kodini kiriting:", parse_mode="HTML")
    await state.set_state(PartPostState.media_id)

@dp.message(PartPostState.media_id)
async def part_post_media_id(message: types.Message, state: FSMContext):
    query = message.text.strip()
    try:
        code = int(query)
...
async def part_post_select_part(callback: CallbackQuery, state: FSMContext):
    part_id = int(callback.data.split("_")[2])
    await state.update_data(part_id=part_id)
    cursor.execute("SELECT p.part_number, m.name, m.code FROM parts p JOIN media m ON p.media_id = m.id WHERE p.id = ?", (part_id,))
    part_num, media_name, code = cursor.fetchone()
    
    post = format_part_post(media_name, part_num, code)
...
async def part_post_channel(message: types.Message, state: FSMContext):
    channel_input = message.text.strip()
    if channel_input.startswith("https://t.me/"):
        channel = "@" + channel_input.split("/")[-1]
    elif channel_input.startswith("@"):
    ...
    post = format_part_post(media_name, part_num, media_code)
...
async def part_post_confirm(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        if media_image:
            await bot.send_photo(chat_id=channel, photo=media_image, caption=post, reply_markup=keyboard, parse_mode="HTML")
        else:
            await bot.send_message(chat_id=channel, text=post, reply_markup=keyboard, parse_mode="HTML")
        try:
...
async def main():
    print("=" * 50)
    print("🎬 Animedia FUN DUB Bot ishga tushmoqda...")
    print("=" * 50)
    print(f"👑 Ownerlar: {OWNERS}")
